# UI coherence fix — shard shard-011
...
3. `.audit/findings/shard-011.md` — the findings for this shard.
...
3. **Always** write `.audit/findings/shard-011.fix.md`, even when nothing was applied:

```
# Fix log — shard-011
| Finding | Status | Detail |
|---|---|---|
| shard-011-F01 | applied | `old-class` → `new-class` in file:line |
| shard-011-F02 | skipped | reason (needs-ruling / out-of-shard: … / reference file / would delete handler / low confidence / …) |
…
...
## Shard files
- `src/widgets/fleet-expenses-table/cash-in-review.tsx` (243 lines)
- `src/widgets/fleet-expenses-table/ledger-list.tsx` (657 lines)
- `src/widgets/fleet-expenses-table/party-picker.tsx` (320 lines)
- `src/widgets/fleet-expenses-table/split-editor.tsx` (481 lines)
- `src/entities/fuel-event/api.ts` (47 lines)
- `src/entities/fuel-event/defaults.ts` (16 lines)
- `src/entities/fuel-event/queries.ts` (105 lines)
- `src/entities/fuel-event/schemas.ts` (115 lines)
- `src/widgets/fuel-event-form/fuel-event-form.tsx` (496 lines)
