# UI coherence fix — shard shard-017
...
3. `.audit/findings/shard-017.md` — the findings for this shard.
...
3. **Always** write `.audit/findings/shard-017.fix.md`, even when nothing was applied:

```
# Fix log — shard-017
| Finding | Status | Detail |
|---|---|---|
| shard-017-F01 | applied | `old-class` → `new-class` in file:line |
| shard-017-F02 | skipped | reason (needs-ruling / out-of-shard: … / reference file / would delete handler / low confidence / …) |
…
...
## Shard files
- `src/features/tracking/api.ts` (132 lines)
- `src/features/tracking/playback.ts` (103 lines)
- `src/features/tracking/schemas.ts` (174 lines)
- `src/features/tracking/tracking-page.tsx` (673 lines)
- `src/features/tracking/url.ts` (74 lines)
- `src/features/tracking/use-history.ts` (441 lines)
- `src/features/tracking/use-live-fleet.ts` (222 lines)
