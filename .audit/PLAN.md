...
- 2026-08-29 — **Baselines: all 46 routes captured** (`e2e/__screenshots__/chromium/*.png`, 24 of them taller than 900 px; `fleet-expenses-messages` and the `/fleet-expenses/review` redirect hit the 8000 px cap). Verify run: **46/46 pass** on immediate re-run (`.audit/baseline-visual-run4-verify.txt`). Earlier 900 px baselines were also 46/46 deterministic (`run2`). Runs take ~1.7 min. Map/ETIT routes are baselined in their error/empty state until the key referrer and CORS are fixed — re-baseline those after the fix. **Port:** the detector runs on 5199 (see incident log); never point it at 5173/5174 while other tooling may hold them.
- 2026-08-29 (night) — **Overnight run incident log.** (1) First launch: every headless call refused — Claude usage window exhausted ("session limit · resets 5:40pm"); runner now waits 15 min and retries transient failures. (2) Second launch: shards 001 and 002 audited and fixed fine (8 and 21 findings) but both failed the visual gate with 45/46 routes changed — cause: another tool (the `understand-anything` plugin's own Vite dashboard, started ~15:55 by a different session) had taken ports 5173 **and** 5174; Playwright's `reuseExistingServer` screenshotted its "Access Token Required" page. The dashboard baseline had already been replaced by that page during the C-B2 re-baseline (commit 3fc3585) and matched itself. Fixes: the change detector now runs its own Vite on **port 5199** with `reuseExistingServer: false` (a busy port fails loudly), the spec re-keys `storageState` to whatever origin it runs on, the runner refuses a foreign holder of the port and treats any route changing ≥50% of its pixels as a crash, and — because the app reads live production data — **each shard now re-baselines right before its fix** so the post-fix diff isolates the fix from data drift. All 46 baselines re-taken on 5199. Shards 001/002 keep their findings and re-run only the fix phase. (3) 02:54 — shard-003 (`shared/ui`) was reverted because a legitimate primitive fix moved 14 routes and the 12-route cap fired; the cap is now off by default (`AUDIT_MAX_CHANGED_ROUTES=46`) and the ≥50%-per-route crash guard is the protection. shard-001 landed (`ea917be`, 7 findings, 0 visual changes).
